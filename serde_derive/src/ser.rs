...
pub fn expand_derive_serialize(input: &mut syn::DeriveInput) -> syn::Result<TokenStream> {
    ...
    let serialize_owned_impl = if cont.attrs.serialize_owned() && cont.attrs.remote().is_none() {
        let owned_params = ParametersOwned::new(&cont);
        let (owned_impl_generics, owned_ty_generics, owned_where_clause) =
            owned_params.generics.split_for_impl();
        let owned_body = Stmts(serialize_owned_body(&cont, &owned_params));
        quote! {
            #[automatically_derived]
            #allow_deprecated
            impl #owned_impl_generics _serde::ser::SerializeOwned for #ident #owned_ty_generics #owned_where_clause {
...
/// Parameters for SerializeOwned implementation generation.
struct ParametersOwned {
    /// Variable holding the value being serialized (always `self` for owned).
    self_var: Ident,

    /// Path to the type the impl is for.
    this_type: syn::Path,

    /// Same as `this_type` but using `::<T>` for generic parameters.
    this_value: syn::Path,

    /// Generics including bounds for the impl.
    generics: syn::Generics,
...
impl ParametersOwned {
    fn new(cont: &Container) -> Self {
        let self_var = Ident::new("self", Span::call_site());
        let this_type = this::this_type(cont);
        let this_value = this::this_value(cont);
        let is_packed = cont.attrs.is_packed();
        let generics = build_generics_owned(cont);
    ...
    /// Type name to use in error messages.
    fn type_name(&self) -> String {
        self.this_type.segments.last().unwrap().ident.to_string()
    }
}

/// Build generics for SerializeOwned implementation.
/// Uses SerializeOwned bound instead of Serialize for field types.
fn build_generics_owned(cont: &Container) -> syn::Generics {
    let generics = bound::without_defaults(cont.generics);
    ...
    match cont.attrs.ser_bound() {
        ...
        None => bound::with_bound(
            cont,
            &generics,
            needs_serialize_owned_bound,
            &parse_quote!(_serde::ser::SerializeOwned),
        ),
    }
}

/// Fields that need a SerializeOwned bound.
fn needs_serialize_owned_bound(field: &attr::Field, variant: Option<&attr::Variant>) -> bool {
    !field.skip_serializing()
        && field.serialize_with().is_none()
        && field.ser_bound().is_none()
        && variant.map_or(true, |variant| {
...
/// Generate the body for serialize_owned method.
fn serialize_owned_body(cont: &Container, params: &ParametersOwned) -> Fragment {
    if cont.attrs.transparent() {
        serialize_owned_transparent(cont, params)
    } else if let Some(type_into) = cont.attrs.type_into() {
        serialize_owned_into(params, type_into)
    } else {
        match &cont.data {
            Data::Enum(variants) => serialize_owned_enum(params, variants, &cont.attrs),
            Data::Struct(Style::Struct, fields) => {
                serialize_owned_struct(params, fields, &cont.attrs)
            }
            Data::Struct(Style::Tuple, fields) => {
                serialize_owned_tuple_struct(params, fields, &cont.attrs)
            }
            Data::Struct(Style::Newtype, fields) => {
                serialize_owned_newtype_struct(params, &fields[0], &cont.attrs)
            }
...
fn serialize_owned_transparent(cont: &Container, params: &ParametersOwned) -> Fragment {
    ...
    let path = match transparent_field.attrs.serialize_with() {
        ...
        None => {
            ...
            quote_spanned!(span=> _serde::ser::SerializeOwned::serialize_owned)
        }
    };
...
fn serialize_owned_into(params: &ParametersOwned, type_into: &syn::Type) -> Fragment {
    let self_var = &params.self_var;
    quote_block! {
        _serde::ser::SerializeOwned::serialize_owned(
            _serde::#private::Into::<#type_into>::into(#self_var),
            __serializer)
    }
}

fn serialize_owned_newtype_struct(
    params: &ParametersOwned,
    field: &Field,
    ...
    let self_var = &params.self_var;
    let mut field_expr = quote!(#self_var.0);
    if let Some(path) = field.attrs.serialize_with() {
        field_expr = wrap_serialize_field_with_owned(params, field.ty, path, &field_expr);
    }
...
fn serialize_owned_tuple_struct(
    params: &ParametersOwned,
    fields: &[Field],
    cattrs: &attr::Container,
) -> Fragment {
    let serialize_stmts = serialize_owned_tuple_struct_visitor(
        fields,
        params,
        false,
    ...
    let len = serialized_fields
        .map(|(i, field)| match field.attrs.skip_serializing_if() {
            ...
            Some(path) => {
                ...
                let self_var = &params.self_var;
                let field_expr = quote!(&#self_var.#index);
...
fn serialize_owned_struct(
    ...
    if has_non_skipped_flatten {
        serialize_owned_struct_as_map(params, fields, cattrs)
    } else {
        serialize_owned_struct_as_struct(params, fields, cattrs)
    }
}

fn serialize_owned_struct_as_struct(
    params: &ParametersOwned,
    fields: &[Field],
    cattrs: &attr::Container,
) -> Fragment {
    let serialize_fields =
        serialize_owned_struct_visitor(fields, params, false, &StructTrait::SerializeStruct);

    let type_name = cattrs.name().serialize_name();

    let tag_field = serialize_struct_tag_field_owned(cattrs, &StructTrait::SerializeStruct);
    ...
    let len = serialized_fields
        .map(|field| match field.attrs.skip_serializing_if() {
            ...
            Some(path) => {
                ...
                let field_expr = quote!(&#self_var.#member);
                quote!(if #path(#field_expr) { 0 } else { 1 })
            }
        })
        .fold(
...
fn serialize_owned_struct_as_map(
    params: &ParametersOwned,
    fields: &[Field],
    cattrs: &attr::Container,
) -> Fragment {
    let serialize_fields =
        serialize_owned_struct_visitor(fields, params, false, &StructTrait::SerializeMap);

    let tag_field = serialize_struct_tag_field_owned(cattrs, &StructTrait::SerializeMap);
    let tag_field_exists = !tag_field.is_empty();
...
fn serialize_struct_tag_field_owned(
    cattrs: &attr::Container,
...
fn serialize_owned_enum(
    ...
    let mut arms: Vec<_> = variants
        ...
        .map(|(variant_index, variant)| {
            serialize_owned_variant(params, variant, variant_index as u32, cattrs)
        })
        .collect();

    if cattrs.non_exhaustive() {
        arms.push(quote! {
            _ => _serde::#private::Err(_serde::ser::Error::custom("cannot serialize non-exhaustive variant")),
        });
    }
...
fn serialize_owned_variant(
    ...
    if variant.attrs.skip_serializing() {
        ...
        let case = match variant.style {
            ...
            Style::Newtype => {
                quote! {
                    #this_value::#variant_ident(__field0)
                }
            }
            Style::Tuple => {
        ...
        let body = Match(match (cattrs.tag(), variant.attrs.untagged()) {
            (attr::TagType::External, false) => {
                serialize_owned_externally_tagged_variant(params, variant, variant_index, cattrs)
            }
            (attr::TagType::Internal { tag }, false) => {
                serialize_owned_internally_tagged_variant(params, variant, cattrs, tag)
            }
            (attr::TagType::Adjacent { tag, content }, false) => {
                serialize_owned_adjacently_tagged_variant(
                    params,
                    variant,
                    cattrs,
                    variant_index,
            ...
            (attr::TagType::None, _) | (_, true) => {
                serialize_owned_untagged_variant(params, variant, cattrs)
            }
        });
...
fn serialize_owned_externally_tagged_variant(
    params: &ParametersOwned,
    variant: &Variant,
    variant_index: u32,
    cattrs: &attr::Container,
    ...
    if let Some(path) = variant.attrs.serialize_with() {
        let ser = wrap_serialize_variant_with_owned(params, path, variant);
        return quote_expr! {
    ...
    match effective_style(variant) {
        ...
        Style::Newtype => {
            ...
            if let Some(path) = field.attrs.serialize_with() {
                field_expr =
                    wrap_serialize_field_with_owned(params, field.ty, path, &field_expr);
            }
        ...
        Style::Tuple => serialize_owned_tuple_variant(
        ...
        Style::Struct => serialize_owned_struct_variant(
...
fn serialize_owned_internally_tagged_variant(
    params: &ParametersOwned,
    variant: &Variant,
    cattrs: &attr::Container,
    tag: &str,
...
fn serialize_owned_adjacently_tagged_variant(
    params: &ParametersOwned,
    variant: &Variant,
    cattrs: &attr::Container,
    variant_index: u32,
...
fn serialize_owned_untagged_variant(
    params: &ParametersOwned,
    variant: &Variant,
    cattrs: &attr::Container,
) -> Fragment {
    ...
    match effective_style(variant) {
        ...
        Style::Tuple => serialize_owned_tuple_variant(TupleVariant::Untagged, params, &variant.fields),
        Style::Struct => {
            let type_name = cattrs.name().serialize_name();
            serialize_owned_struct_variant(StructVariant::Untagged, params, &variant.fields, type_name)
        }
    }
}

fn serialize_owned_tuple_variant(
    ...
    let serialize_stmts = serialize_owned_tuple_struct_visitor(fields, params, true, &tuple_trait);
...
fn serialize_owned_struct_variant(
    ...
    if fields.iter().any(|field| field.attrs.flatten()) {
        return serialize_owned_struct_variant_with_flatten(context, params, fields, name);
    }
    ...
    let serialize_fields = serialize_owned_struct_visitor(fields, params, true, &struct_trait);
...
fn serialize_owned_struct_variant_with_flatten(
    context: StructVariant,
    params: &ParametersOwned,
    fields: &[Field],
    name: &Name,
...
fn serialize_owned_tuple_struct_visitor(
    fields: &[Field],
    params: &ParametersOwned,
    is_enum: bool,
...
fn serialize_owned_struct_visitor(
    fields: &[Field],
    params: &ParametersOwned,
    is_enum: bool,
    struct_trait: &StructTrait,
...
fn wrap_serialize_field_with_owned(
    params: &ParametersOwned,
    field_ty: &syn::Type,
    serialize_with: &syn::ExprPath,
    field_expr: &TokenStream,
) -> TokenStream {
    wrap_serialize_with_owned(params, serialize_with, &[field_ty], &[quote!(#field_expr)])
}

fn wrap_serialize_variant_with_owned(
    params: &ParametersOwned,
    serialize_with: &syn::ExprPath,
    variant: &Variant,
) -> TokenStream {
