...
pub trait Deserializer<'de>: Sized {
    ...
    /// Deserialize a sequence, returning a vector of elements.
    ///
    /// This method provides a more ergonomic way to deserialize sequences by
    /// returning a `Vec<T>` directly, which can then be used with standard
    ...
    ///         D: Deserializer<'de>,
    ///     {
    ///         let bars = deserializer.deserialize_iter::<Bar>()?;
    ///         Ok(Foo(bars))
    ///     }
    /// }
    /// ```
    ...
    fn deserialize_iter<T>(self) -> Result<crate::lib::Vec<T>, Self::Error>
    where
        T: Deserialize<'de>,
    {
        ...
        impl<'de, T> Visitor<'de> for IterVisitor<T>
        where
            T: Deserialize<'de>,
        {
            type Value = crate::lib::Vec<T>;
            ...
            where
                A: SeqAccess<'de>,
            {
                let mut results = crate::lib::Vec::new();
        ...
        self.deserialize_seq(IterVisitor::<T> {
            _marker: PhantomData,
        })
    }
...
///
/// This adapter allows using a `SeqAccess` as a standard Rust [`Iterator`],
/// which enables ergonomic collection of sequence elements using iterator
/// combinators like `collect()`.
///
/// # Lifetime
///
...
///             }
///
///             fn visit_seq<A>(self, seq: A) -> Result<Foo, A::Error>
///             where
///                 A: SeqAccess<'de>,
///             {
    ...
    T: Deserialize<'de>,
{
    ...
    ///
    /// ```edition2021
    /// # use serde::de::{SeqAccess, SeqAccessIterator};
    /// # fn example<'de, A: SeqAccess<'de>>(seq: A) {
    /// let iter: SeqAccessIterator<'de, A, i32> = SeqAccessIterator::new(seq);
    /// # }
    /// ```
    #[inline]
    ...
    /// Returns the underlying `SeqAccess`, consuming this iterator.
    ///
    /// This can be useful if you need to access the `SeqAccess` after
    /// partially iterating through the sequence.
    #[inline]
